...
#' subtitle: <font size="2">Peter Mikula, Martin Bulla, Daniel T. Blumstein, Yanina Benedetti, Kristina Floigl, Jukka Jokimäki, Marja-Liisa Kaisanlahti-Jokimäki, Gábor Markó, Federico Morelli, Anders Pape Møller, Anastasiia Siretckaia, Sára Szakony, Michael A. Weston, Farah Abou Zeid, Piotr Tryjanowski & Tomáš Albrecht</font><br><br>
#' author: <font size="2">created by Martin Bulla</font>
...
#' ### Introduction
#' To facilitate transparency the following document contains all code used to generate the results of the manuscript. Thus, apart from the supplementary figures and tables, below we display also main text figures.but also containts main text figures. The figures and tables are order according to their appearance in the main text. The code is displayed upon clicking the "code" at the top right above each display item.
#' 
#'---
#' 
#' ### Effect sizes for FID ~ Period
# predictions
# full
...
#show_col(col_)
#' <a name="F_1">
#' **Figure 1 | Change in avian tolerance towards humans before vs during the COVID-19 shutdowns.**</a> The dots with horizontal lines represent estimated standardised effect size and their 95% confidence intervals. For the country-specific and „All“ the effect sizes and 95% confidence intervals come from the joint posterior distribution of 5000 simulated values generated by the sim function from the arm package (Gelman et al., 2022) using the mixed model outputs controlled for starting distance of the observer (filled circles) or not (empty circles; Tables [S1](T_S1) and [S2a](T_S2a)). The models were further controlled for flock size, body size, temperature (also a proxy for a day within the breeding season: r Pearson = 0.48; Fig. [S2](F_S2)), and time of a day, as well as for the non-independence of data points by fitting random intercepts of year, weekday, genus, species, species at a given day and year, country (in All - a global mixed model), site, and species within a site, while fitting Period as random slope within site (i.e. allowing for different Period effect at each site) and inn All also within country. Fitting Period as random slope at other random intercepts produces similar results (see Fig. [S1a](F_S1)). The multicollinearity was small as correlations between predictors were weak (Fig. [S2](F_S2)). For the “Combined“, the estimate and 95% confidence interval represent the meta-analytical mean based on the country estimates and their standard deviation (from the country-specific models), and sample size per country. Note that all estimates centre around zero.
#' 
#' <a name="T_S1">
...
#' Continuous variables were z-transformed. For model descriptions see legend of [Figure 1](F_1).
#' 
...
#'
    ...
    out1 %>%
       kbl() %>%
       kable_paper("hover", full_width = F)
#' Continuous variables were z-transformed (scale). For model descriptions see legend of [Figure 1](F_1). Note that (1a) model is the one reported in the main text.
#'
#' <a name="T_S2b">
#' **Table S2b | Alternative models on escape distance given Stringency** </a>
    ...
    out2 %>%
      kbl() %>%
      kable_paper("hover", full_width = F)
#' Continuous variables were z-transformed (scale). For model descriptions see legend of Figure [S6](F_S6). Note that (1a) model is the one reported in the main text.
#' 
#' <a name="T_S2c">
...
#'
#' ### Species-site specific distribution of escape distance across years
...
#' ### Between/within-species variation in escape distance (species means per site)
...
#' <a name="F_4">
#' **Figure 4 | Changes in human presence in parks within and between years and countries.**</a> Left plots represent distribution (histograms) of human presence (Google Mobility), middle plots the raw data, right plots locally estimated scatterplot smoothing. Note that Google Mobility data are not freely available for years before the COVID-19 pandemic (i.e. before 2020) but 2022 was a year without shutdowns in the studied countries. For weekday-specific pattern see [Fig. S5](F_S5) below.
#'
...
#' 
#' ### Effect sizes for  escape distance ~ stringency index or Google Mobility
...
#' Continuous variables were z-transformed.
#'
#' ### Species trends in esccap distance give stringency index or Google Mobility
